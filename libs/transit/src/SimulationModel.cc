...
SimulationModel::SimulationModel(IController& controller)
  ...
  entityFactory.AddFactory(new BatteryStationFactory());
}
...
/**
 * @brief tool for testing instanceof, used in createEntity
*/
...
IEntity* SimulationModel::createEntity(JsonObject& entity) {
  ...
  if (myNewEntity = entityFactory.CreateEntity(entity)) {
    ...
    if (instanceof<BatteryStation>(myNewEntity)) {
      batteryStations.push_back(reinterpret_cast<BatteryStation*>(myNewEntity));
      std::cout << "battery station has been added to vector" << std::endl;
    }
  }
...
void SimulationModel::FindStations() {
  for (auto& [id, entity] : entities) {
    if (BatteryStation* b = dynamic_cast<BatteryStation*>(entity)) {
      batteryStations.push_back(b);
      std::cout << "BATTERY STATION HAS BEEN ADDED TO VECTOR" << std::endl;
    }
  }
}
